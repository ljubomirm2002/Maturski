...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 02, 2020 at 01:36 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addCategory` (IN `title` VARCHAR(255))  NO SQL
BEGIN
SET @query='INSERT INTO categories(title,verified) VALUES(?,0)';
SET @title=title;
PREPARE p FROM @query;
EXECUTE p USING @title;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `addSubcategory` (IN `id` INT, IN `title` VARCHAR(255))  NO SQL
BEGIN
SET @query='INSERT INTO subcategories(title,category_id,verified) VALUES(?,?,0)';
SET @title=title;
SET @id=id;
PREPARE p FROM @query;
...
BEGIN
SET @query='INSERT INTO categories(title,verified) VALUES(?,1)';
SET @title=title;
PREPARE p FROM @query;
EXECUTE p USING @title;
END$$
...
BEGIN
SET @query='INSERT INTO subcategories(title,category_id,verified) VALUES(?,?,1)';
SET @title=title;
SET @id=id;
PREPARE p FROM @query;
EXECUTE p USING @title,@id;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `createUser` (IN `name` VARCHAR(250), IN `username` VARCHAR(250), IN `password` VARCHAR(250), IN `email` VARCHAR(250), IN `address` VARCHAR(250), IN `biography` TEXT)  NO SQL
BEGIN
...
BEGIN
SET @sql='SELECT * FROM categories WHERE verified=1';
PREPARE s FROM @sql;
EXECUTE s;
END$$
...
BEGIN
SET @sql='SELECT * FROM subcategories WHERE category_id=? AND verified=1';
PREPARE a FROM @sql;
SET @id=id;
EXECUTE a USING @id;
END$$
...
BEGIN
IF (EXISTS(SELECT id FROM target WHERE title=a COLLATE utf8_unicode_ci)) THEN
SELECT id FROM target WHERE title=a;
ELSE
INSERT INTO target(title) VALUES (a);
SELECT id FROM target WHERE title=a COLLATE utf8_unicode_ci;
END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getUnverifiedCategories` ()  NO SQL
BEGIN
SET @sql='SELECT * FROM categories WHERE verified=0';
PREPARE s FROM @sql;
EXECUTE s;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getUnverifiedSubcategories` ()  NO SQL
BEGIN
SET @sql='SELECT * FROM subcategories WHERE verified=0 ';
PREPARE s FROM @sql;
EXECUTE s;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `verifyCategory` (IN `id` INT)  NO SQL
BEGIN
SET @sql='UPDATE categories SET verified=1 WHERE id=?';
SET @id=id;
PREPARE s FROM @sql;
EXECUTE s USING @id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `verifySubcategory` (IN `id` INT)  NO SQL
BEGIN
SET @sql='UPDATE subcategories SET verified=1 WHERE id=?';
SET @id=id;
PREPARE s FROM @sql;
EXECUTE s USING @id;
END$$
...
(26, 1000, '2020-06-02', '', 13, 2, 1, NULL, 1);
...
(3, 'Blank', 'blank', 'pravljenjesajta1@gmail.com', 'blank', 'None', NULL, 'aa', 2),
(4, 'Pera', 'pera', 'pera@gmail.com', 'pera', 'per', NULL, '', 1),
(6, 'Pera', 'pera', 'debil@gmail.com', 'peric', 'a', NULL, '', 2),
(7, 'Mika', 'mika', 'mika@gmail.com', 'lazic', 'Nema', NULL, '', 2);
